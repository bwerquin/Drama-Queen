...
export async function isQueenV2Survey(questionnaireUrl: string | null): Promise<boolean> {
    //By default, we redirect to queen v1 app
    if (questionnaireUrl === null) return false;
    const { lunaticModelVersion } = await fetcher<LunaticSource>(questionnaireUrl);
